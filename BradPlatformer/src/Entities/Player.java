...
public class Player{
	...
	public void tick() {
		...
		if((this.x == Level1.player3.x + Player3.width) && (this.y < Level1.player3.y + Player3.height && this.y + this.height > Level1.player3.y))
			leftStop = true;
		else if(this.x > Level1.player3.x) leftStop = false;
		
		if((this.x + this.width == Level1.player3.x) && (this.y < Level1.player3.y + Player3.height && this.y + this.height > Level1.player3.y))
			rightStop = true;
		else if(this.x < Level1.player3.x + Player3.width) rightStop = false;
		...
		if(rightRangedAttack && !projectileExists){
			...
			Level1.bullet.x = this.x + this.width;
			Level1.bullet.y = this.y + 30;
			Sounds.Sounds("src/Resources/res/hit_marker.wav");
		}
		if(leftRangedAttack && facingLeft && !projectileExists){
			projectileExists = true;
			Level1.bullet.x = this.x;
			Level1.bullet.y = this.y + 30;
			Sounds.Sounds("src/Resources/res/hit_marker.wav");
		}
		if(Level1.bullet.x + Level1.bullet.width <= 0 || Level1.bullet.x >= GamePanel.WIDTH) projectileExists = false;
	...
	public void keyPressed(int k) {
		...
		if(k == KeyEvent.VK_W && jumpCount < 2) jumping = true;
		if(k == KeyEvent.VK_S && facingRight && !leftRangedAttack) rightRangedAttack = true;
		if(k == KeyEvent.VK_S && facingLeft && !rightRangedAttack) leftRangedAttack = true;
